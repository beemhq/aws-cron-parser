import { getDaysOfMonthFromDaysOfWeek, getDaysOfMonthForL, getDaysOfMonthForW, arrayFindLast as find } from './common';
...
const findOnce = (parsed: ParsedCron, from: Date): Date | null => {
    ...
    const month = find(parsed.months, (c: number) => c <= (year === cYear ? cMonth : 12));
    if (!month) {
    ...
    const dayOfMonth = find(pDaysOfMonth, (c: number) => c <= (isSameMonth ? cDayOfMonth : 31));
    if (!dayOfMonth) {
    ...
    const hour = find(parsed.hours, (c: number) => c <= (isSameDate ? cHour : 23));
    if (typeof hour === 'undefined') {
        return findOnce(parsed, new Date(Date.UTC(year, month - 1, dayOfMonth) - 1));
    }

    const minute = find(parsed.minutes, (c: number) => c <= (isSameDate && hour === cHour ? cMinute : 59));
 ...
 */
export function prev(parsed: ParsedCron, from: Date) {
